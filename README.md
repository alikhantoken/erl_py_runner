...
- **Protocol:** 4-byte big-endian length-prefixed ETF over stdin/stdout; max message size 10 MB. Message types: init, load/delete library, exec, result (ok/error), and synchronous Erlang call requests from Python.
- **Library management:** Named Python modules can be loaded into all workers at runtime via `load_library/2` and removed via `delete_library/1`. Loaded libraries persist across `run/2,3,4` calls and can be imported by name inside any snippet.
...
| `worker.config.exec_timeout` | Default execution timeout in milliseconds for `run/2,3` | `60000` |
| `worker.config.operation_timeout` | Timeout in milliseconds for load/delete library and worker handshake operations | `30000` |
| `worker.config.max_pending` | Max queued requests when all workers are busy (`infinity` or a non-negative integer) | `infinity` |
| `worker.config.restart_delay` | Milliseconds a worker sleeps in `init` before opening its port | `0` |
| `worker.config.max_code_size` | Maximum allowed byte size for library code passed to `load_library/2` | `1048576` |
...
| `worker.modules_whitelist.python_modules` | Python modules importable in scripts (`all` to allow any) | `all` |
...
%% Result = 30, State = undefined

%% Pass state from a previous call
{ok, Result2, State2} = erl_py_runner:run(
  <<"result = state + arguments['delta']">>,
  #{<<"delta">> => 5},
  Result
).

%% Execution with explicit timeout
{ok, Result3, State3} = erl_py_runner:run(
...
**`erl_py_runner:run/2,3,4`**
```erlang
-spec run(Code :: binary(), Arguments :: term()) -> {ok, term(), term()} | {error, term()}.
...
- `State` - any Erlang term from a previous call; available inside the script as `state`. Defaults to `undefined`.
- `Timeout` - optional per-call deadline in milliseconds; overrides `exec_timeout` from config.

**Output**
```erlang
{ok, Result, NewState} %% Successful execution.
{error, Reason}        %% Python exception, timeout, or port failure. Reason consists from a map: #{message => ..., trace => ...}
```

### Managing Libraries
Libraries are named Python modules that are loaded into every worker. Once loaded, any snippet can import them by name.

```erlang
%% Load (or replace) a library across all workers.
ok = erl_py_runner:load_library(<<"my_utility">>, <<"
def add(a, b):
    return a + b
">>).

%% Use it in a snippet.
{ok, Result, _} = erl_py_runner:run(
  <<"import my_utility; result = my_utility.add(3, 4)">>,
  #{}
).
%% Result = 7

%% Remove the library from all workers.
ok = erl_py_runner:delete_library(<<"my_utility">>).
```

**`erl_py_runner:load_library/2`**
```erlang
-spec load_library(Name :: binary(), Code :: binary()) -> ok | {error, term()}.
```
Installs or replaces a Python module across all pool workers. All workers confirm the load before this call returns. `Code` must not exceed `max_code_size`.

**`erl_py_runner:delete_library/1`**
```erlang
-spec delete_library(Name :: binary()) -> ok | {error, term()}.
```
Removes a previously installed library from all pool workers. All workers confirm the removal before this call returns.

### Pool Statistics
```erlang
Stats = erl_py_runner:stats().
%% Returns a map keyed by worker ID, each value is a map with 'erlang' and 'python' sub-maps
```

**`erl_py_runner:stats/0`**
```erlang
-spec stats() -> #{term() => map()}.
```
Returns a live snapshot of every worker in the pool. Each entry contains:
- `erlang` - port-level metrics
- `python` - process-level metrics
Workers that do not respond within the stats timeout are reported as `#{pid => PID, error => timeout | unavailable}`.

### Other API

**`erl_py_runner:restart/0`** - stops and restarts the entire application. Use this when the pool cannot recover on its own; all in-flight requests will be lost.

### Inside the Python Script
Four variables are injected into the execution scope:
- **`arguments`** - the Erlang term sent as the second argument to `run/2,3,4`.
- **`state`** - the Erlang term sent as the third argument to `run/3,4` (defaults to `None`).
...
Dangerous builtins (`eval`, `exec`, `open`, `compile`, `__import__`, `input`, `exit`, `quit`, `breakpoint`) are removed from the execution scope. Imports are restricted to the configured `python_modules` whitelist (or unrestricted when set to `all`).
...
3. Calling `erl_py_runner:run/2,3,4` dispatches the request to an idle worker. If all are busy, the caller blocks until one frees up or the timeout expires.
4. The worker sends code, arguments, and state to Python, which executes it in `exec()` and returns the result.
...
|-----------|------|--------|
| Erlang → Python | Initialization | `{init, #{modules => all \| [Mod], ...}}` |
| Python → Erlang | Init acknowledgement | `ok` |
| Erlang → Python | Load library | `{load_library, Name, Code, Hash, ExpectedVersion, Version}` |
| Erlang → Python | Delete library | `{delete_library, Name, Hash, ExpectedVersion, Version}` |
| Erlang → Python | Code execution | `{exec, Code, Arguments, State}` |
